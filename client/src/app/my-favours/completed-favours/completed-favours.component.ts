...
import { CompletedFavourCardComponent } from "./completed-favour-card/completed-favour-card.component";
...
@Component({
  selector: 'app-completed-favours',
  imports: [CompletedFavourCardComponent, MatPaginatorModule, AsyncPipe, MatDivider],
  templateUrl: './completed-favours.component.html',
  styleUrl: './completed-favours.component.scss',

})
export class CompletedFavoursComponent {
  ...
  readonly menuItems$ = combineLatest([
    ...
    switchMap(([searchTerm, currentPage]) =>
      this.favourService.favoursCompletedByMe(searchTerm, currentPage)
    )
  );
